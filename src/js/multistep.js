...
$(nextBtn).on("click", async function () {
    ...
    if ($(form).valid() === true) {
        ...
        let numberStepsCompleted = $('#progressbar li.active').length
        form.querySelector("input[name='steps_completed']").value = numberStepsCompleted;
        console.log(numberStepsCompleted);
        ...
        if (mutation.attributeName === "class") {
            if ($(mutation.target).hasClass("activeStep")) {
                ...
                let downTimer = setInterval(function () {
                    ...
                    document.getElementById("countProgressBar").value = 2 - timeToReset;
